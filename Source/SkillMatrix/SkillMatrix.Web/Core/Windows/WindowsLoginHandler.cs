...
namespace SkillMatrix.Web.Core.Windows
{
    /// <inheritdoc />
    /// <summary>
    ...
    public class WindowsLoginHandler : HttpTaskAsyncHandler, IRequiresSessionState
    {
        ...
        /// <returns></returns>
        private static async Task ExecuteController(HttpContext context, RouteData routeData)
        {
            var wrapper = new HttpContextWrapper(context);
            var handler = new MvcHandler(new RequestContext(wrapper, routeData));

            //var asyncHandler = handler;
            IHttpAsyncHandler asyncHandler = handler;
