...
using SkillMatrix.Common.Resources;
...
namespace SkillMatrix.Common
{
    public partial class Utilities
    {
        ...
        public static string GetPrettyDate(string date)
        {
            ...
            if (DateTime.TryParse(date, out time))
            {
                ...
                if (totalDays == 0)
                {
                    if (totalSeconds < 60)
                    {
                        return WebStrings.Utils_GetPrettyDate_Date_JustNow;
                    }
                    if (totalSeconds < 120)
                    {
                        return WebStrings.Utils_GetPrettyDate_Date_OneMinuteAgo;
                    }
                    if (totalSeconds < 0xe10)
                    {
                        return string.Format(WebStrings.Utils_GetPrettyDate_MinutesAgo, Math.Floor((double)(((double)totalSeconds) / 60.0)));
                    }
                    if (totalSeconds < 0x1c20)
                    {
                        return WebStrings.Utils_GetPrettyDate_Date_OneHourAgo;
                    }
                    if (totalSeconds < 0x15180)
                    {
                        return string.Format(WebStrings.Utils_GetPrettyDate_HoursAgo, Math.Floor((double)(((double)totalSeconds) / 3600.0)));
                    }
                }
                if (totalDays == 1)
                {
                    return WebStrings.Utils_GetPrettyDate_Yesterday;
                }
                if (totalDays < 7)
                {
                    return string.Format(WebStrings.Utils_GetPrettyDate_DaysAgo, totalDays);
                }
                if (totalDays < 0x1f)
                {
                    return string.Format(WebStrings.Utils_GetPrettyDate_WeekAgo, Math.Ceiling((double)(((double)totalDays) / 7.0)));
                }
            }
            return date;
        }
        ...
        public static string GetPrettyDate(DateTime? date)
        {
            ...
            {
                var d = date.Value;
                var span = DateTime.UtcNow.Subtract(d);
                var totalDays = (int) span.TotalDays;
                var totalSeconds = (int) span.TotalSeconds;
                if ((totalDays < 0) || (totalDays >= 0x1f))
                {
                    return FormatDateTime(d, "dd MMMM yyyy");
                }
                if (totalDays == 0)
                {
                    if (totalSeconds < 60)
                    ...
                    if (totalSeconds < 0xe10)
                    {
                        return string.Format(WebStrings.Utils_GetPrettyDate_MinutesAgo,
                            Math.Floor((double) (((double) totalSeconds) / 60.0)));
                    }
                    if (totalSeconds < 0x1c20)
                    {
                        return WebStrings.Utils_GetPrettyDate_Date_OneHourAgo;
                    ...
                    if (totalSeconds < 0x15180)
                    {
                        return string.Format(WebStrings.Utils_GetPrettyDate_HoursAgo,
                            Math.Floor((double) (((double) totalSeconds) / 3600.0)));
                    }
                }
                if (totalDays == 1)
                {
                ...
                if (totalDays < 0x1f)
                {
                    return string.Format(WebStrings.Utils_GetPrettyDate_WeekAgo,
                        Math.Ceiling((double) (((double) totalDays) / 7.0)));
                }
            }
            return time;
        }
